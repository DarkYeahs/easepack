...
DevServer.prototype.listen = function (compiler) {
  ...
  // cors
  app.use(function middlewareWrapper(req, res, next) {
  ...
  if (typeof config.webpackDevServer === 'object') {
    Object.assign(serverConfig, config.webpackDevServer)
  }

  var devMiddleware = require('webpack-dev-middleware')(compiler, serverConfig);
  ...
  compiler.plugin('compilation', function (compilation) {
    compilation.plugin('html-webpack-plugin-after-emit', function (data, cb) {
      data.isCached || hotMiddleware.publish({ action: 'reload' })
      cb()
    })
  })
