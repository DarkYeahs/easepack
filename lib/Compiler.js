...
Compiler.prototype.run = function (callback, type) {
  ...
  if (!self.options.screwIe8) {
    self.config.plugins.push(new DontScrewIe8Plugin())
  }
  ...
  self.applyPluginsAsync('configuration', err => {
    ...
    if (this.options.useCommonsChunk) {
      ...
      if (_.isObject(this.options.useCommonsChunk)) {
        Object.assign(commonsChunkOptions, this.options.useCommonsChunk)
      }
      this.config.plugins.push(new Webpack.optimize.CommonsChunkPlugin(commonsChunkOptions))
    }
    if (type === 'config') {
