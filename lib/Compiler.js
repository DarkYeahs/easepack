...
Compiler.prototype.run = function (callback, type) {
  ...
  if (this.options.moreDetails) {
    this.config.plugins.push(new Webpack.ProgressPlugin({
  ...
  if (this.options.rules.length > 0) {
    this.config.rules = this.config.rules.concat(this.options.rules);
  }
  ...
  self.applyPluginsAsync('configuration', err => {
    ...
    if (this.options.useCommonsChunk) {
      ...
      if (_.isObject(this.options.useCommonsChunk)) {
        Object.assign(vendorOptions, this.options.useCommonsChunk.vendor)
        Object.assign(manifestOptions, this.options.useCommonsChunk.manifest)
      }
      if (_.isUndefined(this.options.useCommonsChunk.vendor) &&
        _.isUndefined(this.options.useCommonsChunk.manifest)) {
        Object.assign(vendorOptions, this.options.useCommonsChunk)
      }
      this.config.plugins.push(new Webpack.optimize.CommonsChunkPlugin(vendorOptions))
      this.config.plugins.push(new Webpack.optimize.CommonsChunkPlugin(manifestOptions))
    }
    if (type === 'config') {
...
Compiler.prototype.progressHandler = function (percentage, msg, details) {
  if (percentage !== 0) {
    Stats.clear();
  }
  if (msg) {
    Stats.normal(`${Math.floor(percentage * 100)}%: `);
    Stats.normal(`${msg}${details ? ` ${details}` : ''}`);
  }
  switch (msg) {
    case 'sealing':
      this.costTime('AfterCompile');
      break;
    case 'emitting':
      this.costTime('AfterEmit');
      break;
  }
}
