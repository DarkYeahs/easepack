...
HotUpdatePlugin.prototype.apply = function (compiler) {
  ...
  compiler.plugin('done', (stats) => {
    ...
    this.changed = this.changed.concat(Object.keys(assets).filter(k => {
      ...
      var ext = extExpr.exec(k) || [];
      if (!~['map', 'html'].indexOf(ext[1])
        && (hash = utils.getHashDigest(source)) && hash != this.hash[k]) {
...
Server.prototype.refresh = function (change) {
  if (this.wss && change.length) {
    var ext = null;
    var data = ['refresh', {path: []}];

    for (var i = 0, file; (file = change[i]); i++) {
      if ((ext = extExpr.exec(file))) {
        if (~['js', 'html'].indexOf(ext[1])) {
          data[1].apply_js_live = true;
          break;
        }
        if ('css' === ext[1]) {
          data[1].apply_css_live = true;
          data[1].path.push(file);
        }
      }
    }
    data = JSON.stringify(data);
    this.wss.clients.forEach(socket => socket.send(data));
  }
};