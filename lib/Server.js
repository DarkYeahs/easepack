...
Server.prototype.listen = function (webpackCompiler) {
  ...
  // cors
  this.app.use(function middlewareWrapper(req, res, next) {
    res.setHeader('Access-Control-Allow-Origin', '*');
    // res.setHeader('Access-Control-Allow-Credentials', true);
    next()
  });

  const devMiddleware = middleware(webpackCompiler, {
    publicPath: '/',
    noInfo: true,
    quiet: true
  })
  ...
  devMiddleware.waitUntilValid(() => {
    console.log(`> Listening at http://${this.options.ipv4}:${this.port}\n`)
  })
