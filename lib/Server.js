...
function Server(compiler) {
  ...
  if ((opt.dev || opt.mocha) && !opt.webpackDevServer) {
    ...
    for (var i = 0, m; (m = opt.matches[i]); i++) {
      if (m.props.url && ~m.props.url.split('?')[0].indexOf('[hash]')) {
        var err = new Error(`${m.props.url} : Can't use [hash] as name in development (use ?[hash] instead)\n`);
