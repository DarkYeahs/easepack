...
module.exports = function (source) {
  ...
  var template = _template(content, {variable: 'data'});

  content.replace(imageExpr, (m, url) => {
    urlObj += `urlObj[${JSON.stringify(url)}] = require(${JSON.stringify(loaderUtils.urlToRequest(url))});`;
  });

  return `
    ${urlObj}
    exports.default = (${template.source})().replace(${imageExpr.toString()}, (m, url) => urlObj[url] || url);`;

  // content = content.replace(imageExpr, (m, url) => 
  //   `"+require(${JSON.stringify(loaderUtils.urlToRequest(url))})+"`);

  // return `
  //   var _template = require(${loaderUtils.stringifyRequest(this, _template)});
  //   var _process = Object.assign({}, process);
  //   _process.env.NODE_ENV = process.env.NODE_ENV;
  //   exports.default = _template(${content})({process: _process});
  // `;
};