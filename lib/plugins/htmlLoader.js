...
var parser = new Parser({
  ...
  outside: {
    ...
    '<(script|img)(?=\\s+)'(m, t) {
      this.currentTag = t;
      this.currentTagAttr = {attr: ''}; // 记录当前标签的属性
  ...
  inside: {
    '>' () {
      if (this.currentTagAttr.src) {
        this.currentTag === 'script' && this.loc.push({
          type: 'inject_s',
          indent: this.indent,
          start: this.tagIndex,
          inject: `script_inject!${this.currentTagAttr.src}!${this.currentTagAttr.attr}`
        })
        this.loc.push({
          val: this.currentTagAttr.src,
          len: this.currentTagAttr.src.length,
          start: this.currentTagAttr.start,
          type: this.currentTag
        })
      }
      return 'outside';
    ...
    '\\s(crossorigin)(?:=".*?")?' (match) {
      this.currentTagAttr.attr += match
    },
    '(\\ssrc=")(.*?)"' (match, startStr, src, index) {
      this.currentTagAttr.src = src;
      this.currentTagAttr.index = index;
      this.currentTagAttr.start = startStr.length + index;
    }
  }
});
