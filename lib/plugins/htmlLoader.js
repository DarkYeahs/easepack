...
var parser = new Parser({
  outside: {
    ...
    '<(script|img)(?=\\s+)'(m, t) {
      this.currentTag = t;
      this.currentAttr = {attr: ''}; // 记录当前标签的属性
  ...
  inside: {
    '>' () {
      if (this.currentAttr.src) {
        this.currentTag === 'script' && this.loc.push({
          type: 'inject_s',
          indent: this.indent,
          start: this.tagIndex,
          inject: `script_inject!${this.currentAttr.src}!${this.currentAttr.attr}`
        })
        this.loc.push({
          val: this.currentAttr.src,
          len: this.currentAttr.src.length,
          start: this.currentAttr.start,
          type: this.currentTag
        })
      }
      return 'outside';
    },
    '\\s(crossorigin|type)(?:=".*?")?' (match) {
      this.currentAttr.attr += match
    },
    '(\\ssrc=")(.*?)"' (match, startStr, src, index) {
      this.currentAttr.src = src;
      this.currentAttr.index = index;
      this.currentAttr.start = startStr.length + index;
    }
  }
});
...
module.exports = function (source) {
  ...
  content = content.join('').replace(imageExpr, (m, url) => {
    ...
    const urlArgs = scriptTypeExpr.exec(url);
    if (urlArgs) {
    ...
    return `<%=function(){
      try {return require(${req})}catch(e){return ${JSON.stringify(url)}}
    }()%>`;
  });
