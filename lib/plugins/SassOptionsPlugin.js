...
const _options = {};
const patternExpr = /^([.~\w/][^*]+)\/\*\..*$/;
const defaultSpriteUrl = '[path][name].png'
...
SassOptionsPlugin.prototype._genSpriteMapByPattern = function (_pattern, _layout, loaderContext, callback) {
  ...
  self._glob(_pattern, compiler, loaderContext, function (err, files) {
    ...
    var spriteHash = utils.getHashDigest(spritePng, null, null);
    ...
    var assetPath = mainTemplate.applyPluginsWaterfall('asset-path', '[file-path]', {
      url: self.options.spriteUrl || config.matchProps(`${spriteDir}.png`).url || defaultSpriteUrl,
      assetPath: {ext: 'png', hash: '[chunkhash:6]'},
