...
SassOptionsPlugin.prototype.apply = function (compiler) {
  ...
  compiler.plugin('this-compilation', function (compilation, params) {
    ...
    // compilation.options.sassLoader.importer = function (url, prev) {
    //   this._currentPath = Path.join(prev, '..', url)
    //   return url
    // }
    compilation.options.sassLoader.functions = {
      'sprite-map($pattern, $layout:"binary-tree")' (pattern, layout, done) {
        const _layout = layout.getValue()
        const _pattern = pattern.getValue()
        self._genSpriteMapByPattern(_pattern, _layout, compilation, (err, id) => {
      ...
      'sprite($map, $sprite)' (map, sprite, done) {
        ...
        self._getSpriteMapByMap(_map, _sprite, (err, sprite) => {
          if (err) {
      ...
      'sprite-position-x($map, $sprite)' (map, sprite, done) {
        const _map = map.getValue()
        const _sprite = sprite.getValue();
        self._getSpriteMapByMap(_map, _sprite, function (err, sprite) {
          if (err) compilation.errors.push(err)
    ...
    params.normalModuleFactory.plugin('parser', (parser) => {
      parser.plugin('call __sprite_map__', function (expr) {
        if (!expr.arguments[0]) {
          throw new Error('__sprite_map__()')
        }
...
SassOptionsPlugin.prototype._glob = function (pattern, compiler, compilation, callback) {
  ...
  Glob(pattern, {cwd: context}, function (err, result) {
    ...
    async.map(result, function (file, callabck) {
      ...
      compilation.compilationDependencies.push(filePath)
...
SassOptionsPlugin.prototype._genSpriteMapByPattern = function (_pattern, _layout, compilation, callback) {
  var self = this;
  var spriteDir = null;
  ...
  self._glob(_pattern, compiler, compilation, function (err, files) {
