    ...
    compilation.mainTemplate.plugin('asset-path', function (path, data) {
      if (data.chunk) {
        ...
        switch (path) {
          ...
          case '[sprite]':
            url = data.url;
            assetPath = self.spriteAssetPath;
            break;
          case '[fileemit]':
            url = data.url;
            assetPath = {ext: data.chunk.ext};
            break;
        }

        if (url && assetPath) {
          var path = Path.dirname(data.chunk.name);
          assetPath = Object.assign({}, assetPath, {
            name: Path.basename(data.chunk.name),
            path: (path && path != '.') ? `${path}/` : ''
          });
