...
FileEmitPlugin.prototype.apply = function (compiler) {
  ...
  compiler.plugin('emit', function (compilation, callback) {
    ...
    if (compilation.modules.every(function (module) {
        return module.fileDependencies.every(function (file) {
      ...
      inputFileSystem.readFile(realPath, function (err, buffer) {
        ...
        var assetPath = template.applyPluginsWaterfall('asset-path', '[fileemit]', {
          hash: hash, url: self.url, chunk: {name: self.name, hash: hash, ext: ext}
        });
