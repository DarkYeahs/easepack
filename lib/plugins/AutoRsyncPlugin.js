  ...
  compiler.plugin('after-emit', function (compilation, callback) {
    async.waterfall([
      ...
      function (stdout, stderr, callback) {
        var commit = '*';
        var msg = JSON.stringify(self.options.msg || 'Auto commit by easepack');
      ...
      function (stdout, stderr, callback) {
        ...
        if (!path[0])
          return callback(new Error('Auto Rsync : the path `'
            + self.nodePath + '` is not in `svn:htdocs/activity`\n'));

        http.get(url, function (res) {
          callback(res.statusCode == 200 ? null : new Error('http.get url ' + url));
        });
      }
    ], function (error) {
