    ...
    async.waterfall([
      ...
      function (stdout, stderr, callback) {
        ...
        if (self.nodePath !== outputPath) {
          commit = Path.relative(self.nodePath, outputPath);
        }
      ...
      function (stdout, stderr, callback) {
        ...
        if (!path[0])
          return callback(new Error('Auto rsync access url warn: `'
            + self.nodePath + '` is not in `svn:htdocs`\n'));

        http.get(url, function (res) {
          callback(res.statusCode == 200 ? null :
            new Error('Auto rsync access url failed `' + url + '`'));
        });
      }
    ], function (error) {
